...
class pb_backupbuddy_core {
	...
	function send_remote_destination( $destination_id, $file, $trigger = '', $send_importbuddy = false ) {
		...
		if ( $destination['type'] == 's3' ) {
			...
			$response = $this->remote_send_s3( $destination['accesskey'], $destination['secretkey'], $destination['bucket'], $destination['directory'], $destination['ssl'], $file, $destination['archive_limit'], $send_importbuddy );
	...
	function remote_send_dropbox( $token, $directory, $file, $limit = 0, $send_importbuddy = false ) {
		...
		pb_backupbuddy::status( 'details',  'About to put object (the file) to Dropbox cron.' );
	...
	function remote_send_s3( $accesskey, $secretkey, $bucket, $directory = '', $ssl, $file, $limit = 0, $send_importbuddy = false ) {
		...
		$s3 = new pb_backupbuddy_S3( $accesskey, $secretkey );
		
		if ( $ssl != '1' ) {
			pb_backupbuddy_S3::$useSSL = false;
		}
		
		// Set bucket with permissions.
		pb_backupbuddy::status( 'details',  'About to put bucket to Amazon S3 cron.' );
		$s3->putBucket( $bucket, pb_backupbuddy_S3::ACL_PRIVATE );
		pb_backupbuddy::status( 'details',  'About to put object (the file) to Amazon S3 cron.' );
		if ( !empty( $directory ) ) {
			$directory = $directory . '/';
		}
	...
	function remote_send_rackspace( $rs_username, $rs_api_key, $rs_container, $rs_file, $limit = 0, $rs_server, $send_importbuddy = false ) {
		...
		pb_backupbuddy::status( 'details',  'About to put object (the file) to Rackspace cron.' );
