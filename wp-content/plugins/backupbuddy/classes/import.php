	...
	function migrate_htaccess() {
		if ( pb_backupbuddy::$options['skip_htaccess'] == true ) {
			...
			if ( !is_writable( ABSPATH . '.htaccess' ) ) {
				pb_backupbuddy::status( 'error', 'Error #9020: Unable to write to .htaccess file. Verify permissions.' );
				pb_backupbuddy::alert( 'Warning: Unable to write to file .htaccess. Verify this file has proper write permissions. You may receive 404 Not Found errors on your site if this is not corrected. To fix: Log in to your WordPress admin and select Settings:: Permalinks from the left menu then save.', '9020' );
				return false;
			}
			
			// If no .htaccess file exists then create a basic default one then migrate that as needed. @since 2.2.32.
			if ( !file_exists( ABSPATH . '.htaccess' ) ) {
				pb_backupbuddy::status( 'message', 'No .htaccess file found. Creating basic default .htaccess file.' );
...
# END WordPress\n";
			...
			pb_backupbuddy::status( 'message', 'Migrating .htaccess file...' );
			...
			pb_backupbuddy::status( 'message', 'Checking .htaccess file.' );
			...
			if ( $newurl !== $oldurl ) {
				...
				pb_backupbuddy::status( 'message', 'Migrated .htaccess file.' );
			} else {
				pb_backupbuddy::status( 'message', 'No changes needed for .htaccess file.' );
			}
		}
		return true;
